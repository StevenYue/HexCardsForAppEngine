...
$(document).ready(function(){
	...
	$("#outButton").bind("click", outButtonClick);
				...
				for(var i=0;i<data.length;i++){
					...
					if(data[i].indexOf(yourID) >= 0){
						...
						for(p=0;p<ss.length;p++){
							if(ss[p]!=yourID){
								opponentID = ss[p];
								if(opponentID == "" && gameStarted)
									updateMsgWindow("***Opponent OffLine***");
							}
						}
					}
				}
...
function sendLogOffRequest(){
	var jMsg = {"ID": yourID, "ROOMID":roomID, "GAMEID":gameID};
	$.ajax({
		type : 'POST',// GET Or POST
		url  : "/logoff",
		data: jMsg,
